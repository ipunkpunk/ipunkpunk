<h1 align="center">✦Carlos Henrique Gomes Santos✦  👋</h1>

---

## 🚀 Atualmente

- 💻 Técnico em Desenvolvimento de Sistemas.
- 📚 Aprimorando meus conhecimentos em programação e tecnologia.
- 🌐 Estudando desenvolvimento web com HTML, CSS e JavaScript.
- 🐍 Desenvolvendo projetos e exercícios utilizando Python.
- 🗄️ Aprendendo conceitos de banco de dados e lógica de programação.
- 🔐 Explorando a área de TI e cibersegurança.

---

## 🛠️ Conhecimentos

- Estruturação de páginas com HTML5.
- Estilização de interfaces com CSS3.
- Desenvolvimento básico com JavaScript.
- Programação em Python.
- Lógica de programação e algoritmos.
- Conceitos de banco de dados SQL.
- Versionamento utilizando Git e GitHub.
- Noções de redes e sistemas computacionais.

---

## 💡 Sobre Mim

- Tenho interesse em tecnologia e desenvolvimento de software.
- Busco evoluir constantemente minhas habilidades na área de TI.
- Gosto de aprender na prática através de projetos e estudos.
- Interessado em desenvolvimento fullstack e cibersegurança.
- Amo Rock e Cultura Geek

---

## ⚡ Tecnologias

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)

![HTML5](https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white)

![CSS3](https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white)

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
